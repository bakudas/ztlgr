...
mod tests {
    ...
    fn test_markdown_save_creates_file() {
        ...
        assert!(content.contains("Test Note"), "File doesn't contain note title");
        assert!(content.contains("This is test content"), "File doesn't contain note content");
    }
    ...
    fn test_markdown_save_preserves_content() {
        ...
        let storage = MarkdownStorage::new();
        storage.write_note(&note, &note_path)
            .expect("Failed to write note");
        
        let content = std::fs::read_to_string(&note_path)
            .expect("Failed to read written file");
        assert!(content.contains(original_content), 
                "Content not preserved. Original: {}, Found: {}", original_content, content);
    }

    #[test]
    fn test_markdown_save_creates_parent_dirs() {
        let temp_dir = tempdir().expect("Failed to create temp dir");
        let nested_path = temp_dir.path().join("a").join("b").join("c").join("note.md");
        ...
        assert!(nested_path.parent().unwrap().exists(), "Parent directories were not created");
    }
}